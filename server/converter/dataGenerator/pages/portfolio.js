...
const portfolioWorker = (pageData) => {
    ...
    if (pageData.modified) {
        Images.loadImgById(pageData.customFields['photo'][0], Config.PATH.PORTFOLIO_IMAGES + mainImage, true);
    }
};
...
function createPortfolioFile(customFields, mainImage) {
    portfolioData = {
        ...
        page_background: '/' + Config.PATH.PORTFOLIO_IMAGES + mainImage,
...
const writePortfolioFile = () => {
    ...
    Utils.writeJsonFile(Config.PATH.JSON_DATA, 'portfolio.json', portfolioData, true);
    clearData();
};
